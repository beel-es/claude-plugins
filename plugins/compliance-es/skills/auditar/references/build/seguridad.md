# Receta: seguridad (MFA, cifrado en reposo, audit log) — control `sec-*`, art. 32 RGPD

> Stack base: TypeScript/Node (Next.js o NestJS) + Postgres. Versiones verificadas en npm 2026-06-21;
> confirmar la vigente con `npm view`. El art. 32 RGPD pide medidas "apropiadas al riesgo", no una lista
> cerrada: estas son las que mueven la aguja en un SaaS.
...
## 2. Cifrado en reposo a nivel de campo — controles `sec-rest`, `data-pseudonym` (arts. 25, 32)
...
deja la llave en la BD → el admin ve el dato; preferir app-level para lo sensible (DNI, salud, IBAN). La
seudonimización es una medida expresamente citada por el art. 32.1.a) RGPD.

## 3. Audit log + contexto actor/IP — control `sec-logs`
Tabla append-only. Lo que suele faltar: **capturar quién + IP sin cambiar todas las firmas** → patrón
**AsyncLocalStorage** (`node:async_hooks`, nativo).
...
hacer la tabla inmutable (revocar UPDATE/DELETE) y/o encadenar hashes — opcional. Este mismo audit log
sirve de base para el **registro de eventos** de Veri*Factu (ver `facturacion.md`).
