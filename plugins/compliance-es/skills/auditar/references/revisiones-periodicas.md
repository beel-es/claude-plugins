...
El cumplimiento **se degrada solo**: añades una tabla, un proveedor nuevo, una feature que toca datos, y la
postura cambia sin que nadie lo note. Por eso conviene **re-correr la skill cada cierto tiempo**: cada
...
quién se autentica y quién paga** en cada una (se confunde a menudo):
...
Claude Code corre sin interacción con el flag de impresión (`claude -p "..."`), así que entra en un cron del
sistema o un **GitHub Action programado** (no depende de tu máquina).
  ...
  audit:
    ...
    steps:
      ...
      - uses: anthropics/claude-code-action@v1
        with:
          prompt: "audita el cumplimiento de este repo con la skill compliance-es"
        env:
          ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
      # luego: subir el .compliance/ actualizado como artifact o abrir un PR con el diff
```
- **Coste:** pago por token (API), **no** tu suscripción. Ideal si quieres cron real y coste por uso.
...
tu máquina prendida.
- **Cuenta/coste:** corre contra tu **suscripción Claude.ai (Pro/Max/Team)** vía OAuth, **sin API key**.
- **Cuota diaria** de ejecuciones (verificar la vigente en claude.ai/settings/usage).
- **Gotcha:** corre sobre un **clon fresco del repo** (no ve archivos locales) → la skill y el repo deben
  estar versionados.

## Resumen de coste (lo que más se confunde)
...
> **Gotcha global:** si tienes `ANTHROPIC_API_KEY` en el entorno, **manda el API key y te cobra por API
